...
% Kept even in the English report: some quoted event/talk content includes
% Vietnamese words (e.g. "Rủi ro tối thiểu"), which need vntex's T5 font
% support to render at all under pdflatex - without it, pdflatex has no
% glyph for characters like "ủ" and the build fails.
\usepackage{vntex}
...
% vntex sets \contentsname to "Mục lục" via its own \AtBeginDocument hook,
% which runs after the preamble and would silently win over a plain
% \renewcommand placed here. Deferring this one to \AtBeginDocument too
% means it registers after vntex's (since this line comes later in the
% source) and runs last, so "Contents" is what actually gets typeset.
