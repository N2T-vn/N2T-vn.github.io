...
A close second was tracing down why a poster upload was silently failing in
the deployed environment: a response with a `200` status that turned out, on
closer inspection of the headers, to be a fake one - the CDN's own fallback
page standing in for a request WAF had quietly blocked. Finding the real cause
underneath a convincingly misleading success response was a good reminder that
the status code alone is never the whole story.
