...
team over seven weeks, with the backend and frontend developed in parallel
...
- **Amazon S3**: Four private buckets - the built React site (read only by
  CloudFront, never served directly), storage for event posters, storage for
...
The project runs for seven weeks, from 15/06/2026 to 31/07/2026, inside the
wider internship period.

- **Week 1 - Idea, API spec, and database schema.** The project is chosen and
  its central problem defined; the API endpoint list and database schema are
  agreed and frozen together, before any AWS service is studied in depth or
  any code is written.
  *Milestone: the contract frozen.*
- **Week 2 - Account foundations and identity and access management.** AWS
  account setup, MFA, billing alarm, and IAM fundamentals.
- **Week 3 - Compute, storage, and managed databases.** EC2, S3, and RDS
  fundamentals, plus the transaction and locking concepts the booking system
  depends on.
- **Week 4 - Core endpoints, local only.** The backend and frontend are built
  in parallel against the frozen contract, on a local Dockerised database.
  The two endpoints that need real AWS infrastructure (poster upload, ticket
  generation) are deliberately deferred.
  *Milestone: the complete application running on localhost.*
- **Week 5 - Deploying to AWS.** RDS, S3, and EC2 deployed; the single point
  of failure in each of the compute and data tiers is then deliberately
  removed with a second EC2 instance behind a load balancer and a Multi-AZ
  database.
  *Milestone: live on AWS.*
- **Week 6 - AWS-dependent features, CDN, network hardening, and monitoring.**
  The deferred endpoints are implemented; CloudFront and WAF put a single
  HTTPS domain in front of the whole application; the compute tier moves
  behind a NAT gateway with no public IP or inbound SSH; CloudWatch
  dashboards and alarms are configured.
  *Milestone: feature-complete and verified.*
- **Week 7 - Testing, report, and submission.** The concurrency test and edge
  cases are proven against the deployed system; the report is written and
  submitted.
  *Milestone: submitted.*

Buffer time is concentrated in weeks 5 and 6 - the two weeks where
infrastructure work (load balancing, Multi-AZ failover, CDN, private
networking) is heaviest and least predictable - rather than spread evenly
